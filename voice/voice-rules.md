# Regras de voz

> RASCUNHO v0 — validar com a Clara no encontro. A carta de vendas (pendente) deve refinar bastante este arquivo.

## Tom

- **Acolhedor mas direto.** Ela não amacia a verdade, mas entrega com cuidado: "Respeite seu tempo, mas não pare de caminhar."
- **Literário, pé no chão.** Imagens concretas (água, jardim, prato de comida, árvore) em frases curtas. Metáfora a serviço da clareza, nunca ornamento.
- **Pessoal.** Ela aparece no conteúdo: o que viveu, o que errou, o que colheu. Vulnerabilidade real, não performada.

## Construções características

- Fala em **2ª pessoa** com a leitora: "Você também tem medo da rejeição?"
- **Pergunta no fechamento** que devolve a bola: "Faz sentido p você? Me conte aqui 👇"
- **Declarações-âncora** curtas e afirmativas: "A rendição é o caminho e o destino é o amor."
- **Imperativo gentil:** "Derreta-se." / "Não deixe esse medo te silenciar."
- Usa a própria história como prova antes de aconselhar.

## Vocabulário

- **Dela:** essência, postura, clã, sistema, feminino/masculino (energias), rendição, colheita, se autorizar, transbordar, acolher, sentir, padrão, lealdade invisível.
- **Registro:** informal-cuidado. Emojis com parcimônia (✨🌱🌺♥️ — nunca em rajada). Abreviações de fala ("pq", "vc", "q", "p", "qdo", "tb") dominam a escrita espontânea dela (ver anotações em `knowledge-base/materiais-atendimentos/`); nos reels publicados ela escreve por extenso. Padrão do sistema: por extenso em conteúdo final, abreviado é sinal de rascunho dela (PERGUNTAR no encontro se confirma).
- **Imagens didáticas recorrentes:** prato/tempero/chef, água, jardim/flores, árvore genealógica, jogo/jogadores. Conceito abstrato sempre aterrissa numa dessas — preferir estender uma imagem que ela já usa a inventar metáfora nova.

## Estrutura de conteúdo (vídeo-primeiro)

- Ela **grava, não escreve**: roteiros são apoio de fala (gancho + pontos-chave), nunca texto pra decorar.
- Gancho dos 3 primeiros segundos obrigatório no draft.
- Legenda com texto de abertura forte (85% assiste sem som).
- CTA preferido: conversa (comentário, direct) — venda acontece na conversa, não no post.

## Réguas de qualidade

1. Isso poderia ter saído de qualquer perfil do nicho? → reescrever (genérico).
2. Tem contexto concreto (situação real, anonimizada)? → senão, buscar no banco de ideias.
3. Ela falaria isso em sessão? → senão, cortar.
4. Passa no `anti-padroes.md` e no `shared/compliance-cfp-cdc.md`? → obrigatório.
